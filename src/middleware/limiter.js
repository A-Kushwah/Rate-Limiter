...
function rateLimiter(opts = {}) {
  return async function limiter(req, res, next) {
    // IMPORTANT: don't use req.path here. Express strips the mount prefix
    // from req.path/req.url for any middleware/router mounted with
    // app.use('/api', ...) — so inside this middleware, a request to
    // "/api/login" shows up with req.path === "/login". ROUTES_JSON keys
    // are written as full public paths (e.g. "POST /api/login"), so we
    // need the full path as the client sent it: req.originalUrl is
    // mount-independent, we just strip the query string off it.
    const fullPath = req.originalUrl.split('?')[0];
    // Route *matching* (ROUTES_JSON lookup) uses the untruncated path —
    // configured prefixes are short and won't be affected by the cap.
    const routeCfg = resolveRouteConfig(req.method, fullPath);
    const id = resolveId(config.keyStrategy, req);
    const routeScope = opts.scope || `${req.method} ${safeScopePath(fullPath)}`;
